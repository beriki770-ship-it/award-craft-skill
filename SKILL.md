...
- **Gate decorative work on the first human gesture, never on timers.** A teaching animation on a 2.4s timer cost 7 mobile Speed Index points — the lab trace saw it churn. Trigger on first mousemove / scroll / touch instead: lab runs never gesture, humans always do within seconds. Same rule for expensive layout prep (split/absolutize/measure) — do it lazily on first interaction.
- **Trust rendered pixels over font tables.** Calibrating a fallback font via OS/2 xAvgCharWidth was off by 30%; measuring the same sentence in both faces in the browser gave the true size-adjust in one shot.
